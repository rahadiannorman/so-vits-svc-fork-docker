...
RUN apt-get update \
    && apt-get install --no-install-recommends -y python3 python3-dev python3-pip build-essential nano htop nvtop p7zip-full screen rclone \
...
RUN pip install --no-cache-dir -U  torch torchaudio --index-url https://download.pytorch.org/whl/cu118 \
    && pip install --no-cache-dir -U so-vits-svc-fork vastai \
